...
and ship as a skill package for compatible tools. New skills start as
an issue, not a PR.
...
- **Installation parity.** I install from this repo the same way you do. Nothing in it may depend on context that exists only on my machine. That means no absolute paths, no private repo names, and no personal-environment assumptions. Contributions follow the same rule.
