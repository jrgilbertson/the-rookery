...
- **`main` is the install source.** Installs pull from the default branch, so whatever lands on `main` is instantly what people get. `main` stays install-clean. Experiments live on branches.
