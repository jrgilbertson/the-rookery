...
- [managing-issues](skills/managing-issues/SKILL.md). Manage one GitHub or Linear issue, or its native parent and blocker graph, through the repository's canonical tracker. Reads and drafts need no setup; interactive setup before the first mutation records the canonical provider, target, and metadata vocabulary. Cross-tracker requests require one exact provider-native link, and only the canonical tracker is written. The skill returns verified issue, graph, readiness, blocker, and completion facts without beginning implementation.
