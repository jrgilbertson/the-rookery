...
- [managing-issues](skills/managing-issues/SKILL.md). Manage one GitHub or Linear issue, or its native parent and blocker graph, through the repository's canonical tracker. Authenticated GitHub and Linear paths preview effects, apply each approved effect once, and verify its readback. Reads and drafts need no setup; before the first tracker mutation, interactive setup records the operator's provider, target, metadata vocabulary, and whether provider-managed synchronization accepts creates from that canonical tracker. Linear uses connected MCP tools by default when available and also supports an explicitly selected Orca CLI path. The skill writes only the canonical tracker and leaves mirroring to GitHub and Linear's native integration. It returns verified issue, graph, and readiness facts that any downstream IDE or workflow may consume; it does not invoke or persist execution orchestration. It marks completion only against Verification evidence, and the tracker remains the only durable work state.
