...
`checking-pr-readiness` compares the finished branch with the plan, verifies the named evidence, and surfaces unresolved risks before the pull request opens. It briefs a recommendation plus numbered live options and waits for a numbered reply. A reply of 1 approves the exact revision shown and continues into `ce-commit-push-pr` in the same conversation, which writes the evidence into the pull request description and opens the pull request. That reply is the last owner go-ahead. Before approving, I can also ask for an explanation of the change or a concept it introduced.
