...
4. [`no-ai-slop`](https://github.com/petergyang/no-ai-slop) and [`humanize`](https://github.com/harshaneel/humanize) (`ai-check`) when the change includes human-facing prose. `no-ai-slop` sharpens the draft and keeps the writer's voice; `humanize` flags AI-sounding writing.
