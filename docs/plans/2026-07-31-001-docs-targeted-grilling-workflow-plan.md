...
- R7. A grilling session must walk dependent decisions in order, ask one question at a time, recommend a concrete answer with each question, investigate discoverable facts, preserve the user's authority over decisions, and wait for explicit shared-understanding confirmation before action.
...
- F2. Clear-work bypass
  ...
  - **Covers:** R2, R5
- F3. Prototype-informed grilling
  - **Trigger:** A grill reaches a visual or interaction decision that cannot be evaluated reliably in prose.
  - **Actors:** A1, A2
  - **Steps:** The session pauses for a prototype or design exploration; the resulting evidence returns to the same grill; the user then resolves the decision.
  - **Outcome:** High-fidelity questions are answered from something concrete without moving implementation into the grill.
  - **Covers:** R6, R7
...
- AE5. **Covers R6, R7.** Given an interaction-feel decision that cannot be judged from prose, when the grill reaches that decision, then the workflow allows a separate prototype or design exploration and resumes the grill with that evidence.
...
- The documented grill preserves the recommend-and-react interaction and explicit confirmation gate.
...
- KTD4. **Explicit confirmation is the only action boundary.** A pause, abandoned interview, inferred agreement, or prototype result does not pass the Shared Understanding Gate. A separately authorized prototype may gather evidence and return it to the same unresolved question, but the user must still decide and explicitly confirm shared understanding. Governs R6, R7 and F3.
...
- **Conversational momentum is mistaken for confirmation.** Name explicit user confirmation as the only Shared Understanding Gate transition and test pause and abandonment as non-confirmation.
- **A prototype is mistaken for implementation or approval.** Require separate authorization, treat its output as evidence only, and return to the unresolved grill question.
...
- **Goal:** Make the existing Plan section route, run, pause, and exit a targeted Grilling Session without changing the seven-stage structure.
- **Requirements:** R1-R8; F1-F3; AE1-AE5; KTD2-KTD4.
- **Dependencies:** None.
- **Files:** `WORKFLOWS.md`.
- **Approach:**
  ...
  4. Define explicit user confirmation as the Shared Understanding Gate and return confirmed intent to `ce-plan` or the already-running CE planning session.
  5. Add a concise prototype escape hatch: separately authorized design evidence returns to the same Grilling Session and does not settle a decision or authorize implementation.
- **Patterns to follow:** The Plan section's rationale → tool routes → exit gate anatomy; the Design section's existing statement that visual iteration sharpens planning; canonical terms from `CONCEPTS.md`.
- **Test scenarios:**
  ...
  - Covers AE3. Given the operator pauses or abandons the interview without explicit confirmation, the documented route remains blocked from downstream action.
  - Covers F3 / AE5. Given prose cannot resolve an interaction-feel decision, separately authorized prototype evidence returns to the same question and does not pass the Shared Understanding Gate.
...
- **Requirements:** R1-R10; F1-F3; AE1-AE6; KTD6.
- **Dependencies:** U1, U2.
...
- **Test scenarios:**
  ...
  - Given a suggested rewrite that changes `Grilling Session`, `Shared Understanding Gate`, or Compound Engineering's precedence, the executor rejects that rewrite.
...
| Interaction boundary | Run the fact, pause, confirmation, and prototype traces from U1. | The agent investigates facts, asks one recommended parent decision at a time, and performs no downstream action without explicit user confirmation. |
...
- Only explicit user confirmation passes the Shared Understanding Gate; prototype evidence does not settle decisions or authorize implementation.
