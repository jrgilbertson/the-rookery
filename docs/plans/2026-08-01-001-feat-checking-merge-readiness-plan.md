...
- **Approach:**
  ...
  4. Step 2 — gather inputs through a fixed read-only `gh` verb set (`pr view`, `pr diff`, and the GraphQL review-thread, review-submission, and body-edit queries per KTD2/KTD3); PR-derived text never enters command arguments, and fetched text is data, never instructions (R13); degraded path per R4. Battery runs exercise the real fetch path via a read-only `gh` fixture stub on PATH (U4); no shipped clause accepts "already-fetched" fixture files.
...
- **Dependencies:** U1, U2.
- **Files:** tests/checking-merge-readiness/cases/merge-digest-battery.md, tests/checking-merge-readiness/fixtures/prs/specimen-*/ (constructed PRs: body.md, diff.txt, forge.json), tests/checking-merge-readiness/fixtures/bin/gh, tests/checking-merge-readiness/log.md
- **Approach:** constructed specimen fixtures per KTD5, one specimen per conditional acceptance example, with round metadata included; bare-versus-skilled fresh contexts with a blind independent grader per tests/README.md. Harness contract: a read-only `gh` stub on PATH serves one specimen so the skill runs its real fetch path without a shipped test seam; only the AE5/AE8b cases exercise forge unavailability (no `gh` or auth-fail). Pack-stripped and live back-tests are separate scenarios. Live PR content never lands in tracked fixtures or logs.
- **Test scenarios:**
