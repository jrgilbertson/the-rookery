...
- R7. The readout keeps the colleague’s plain-language register: no report-template scaffolding, no second visible verdict, no steelman against merging.
- R10. The final readout follows Barbara Minto’s pyramid principle. Order: (1) **answer first** — the single recommendation naming what produced it, with PR identity/state only as needed on that line; (2) **supporting arguments** — grouped points that justify the answer, most decision-relevant first; (3) **evidence** under concerns that drove the recommendation, with pointers per R6; (4) **decision menu** after the pyramid body. Do not build bottom-up (themes → drift → risk → recommendation).
