...
The existing three chief-of-staff reviews will add a deeper coaching layer without becoming a 22-question portrait. Wind-down will apply at most one decision-useful coaching intervention before tomorrow's Meaningful Commitments lock. Weekly will identify supported recurring threads and turn the highest-leverage one into a falsifiable test or boundary. Quarterly will examine corpus coverage, map supported threads, surface a central tension and compounding strength when evidence permits, and audit whether canonical coaching rules changed later behavior.
...
- R8. Weekly surfaces zero to three recurring threads, each supported by at least two independent temporally distinct observed episodes, checks the bounded corpus for counterevidence or changed behavior, and converts one useful thread into a small falsifiable experiment or boundary with an action, observation window, and support-or-disconfirm signal. When no thread clears that bar, Weekly returns an honest null instead of manufacturing one.
- R9. Quarterly always includes one compact durable-corpus coverage line, expanding gaps or source skew only when they materially affect a conclusion. When evidence supports them, it adds a recurring-thread map, a central tension, and a compounding strength. Its audit classifies relevant canonical coaching rules as applied, repeated without observed behavior change, contradicted, not observed, or inconclusive. For this audit, a canonical rule is an explicit, dated principle, boundary, or heuristic resolved from a configured strategy or learning role; later behavior may be evidenced or cautiously inferred from durable journals, reviews, or optional decision notes. Use `not observed` only when the durable corpus adequately covers the relevant later period but contains no enactment; use `inconclusive` when rule provenance or later coverage is insufficient. Stronger meanings such as ignored or stale require user confirmation.
...
- F3. **Weekly recurring-thread test**
  ...
  - **Flow:** Seek counterevidence or change of course; classify the thread as new, already known, weakened, or unresolved; select the highest-leverage supported thread; propose an action, observation window, and support-or-disconfirm signal for next week.
...
- F4. **Quarterly strategic coaching audit**
  ...
  - **Flow:** State corpus coverage; map supported threads without double-counting reviews and journals; show material counterevidence; surface a central tension and compounding strength only when supported; audit each relevant canonical rule against later behavior.
