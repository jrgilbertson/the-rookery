...
Structural validation and the isolated fixture self-check must pass. `git diff --check` must report no whitespace errors. The status review must show no personal or generated artifact. Preserve the unrelated untracked `.impeccable/` and `PRODUCT.md` paths and exclude them from any commit.
