...
applies_when:
  ...
  - A request starts from a non-canonical tracker
tags:
  - agent-skills
  - issue-management
...
share: the canonical provider, its exact target, and the team's semantic
mappings. The active command path is an execution detail of one session.
Persisting it would make repository state depend on whichever client happened
to be available when setup ran.

Managing Issues applies that separation directly. Its validator accepts only
`version`, `provider`, `target`, and four metadata mapping families. There is no
transport or integration-posture field
...
is unavailable (`skills/managing-issues/references/linear.md`, “Select one
session transport”).
...
Store the operator's canonical provider and exact target and the metadata
vocabulary issue authors choose from. Do not store an IDE, CLI binary, MCP
server, skill name, or transport preference. The validator normalizes version,
provider, target, and mappings as the complete durable
...
(`skills/managing-issues/references/linear.md`, “Select one session transport”).
...
(`skills/managing-issues/references/linear.md`, “Authenticate, resolve, and
discover”). The validator reinforces this boundary by accepting canonical
Linear priority names and rejecting other representations
...
and metadata mappings, then resumes the pending request after
setup completes (`skills/managing-issues/SKILL.md`, “Resolve the tracker and
current facts”). Setup does not ask about or record any integration posture.
...
The configured provider stays canonical, and Managing Issues writes only that
canonical record. A request that starts from another tracker is resolvable only
through one exact provider-native link to the canonical issue; an absent or
ambiguous link requires the exact canonical issue or stops the operation. The
repository stores no sidecar identity map (`skills/managing-issues/SKILL.md`,
“Resolve the tracker and current facts”).

For lifecycle changes, include every effect the canonical provider reports.
Unreported external effects are not observable and never block the operation.
...
- First-use setup may create provider metadata or repository files before the
  requested mutation.
- Cross-tracker requests need one exact native route to the canonical issue.
...
(`skills/managing-issues/references/linear.md`, “Select one session transport”
and “Authenticate, resolve, and discover”).
...
by that guide (`skills/managing-issues/references/linear.md`, “Resolve the Orca
command surface”). Do not rewrite the config to record `orca`.

### First create in an unconfigured repository

Pause before the tracker mutation, ask the operator to choose provider, exact
target, and starter, existing, or custom metadata
mappings (`skills/managing-issues/SKILL.md`, “Resolve the tracker and current
facts”). If selected labels are missing, approve and read back their creation
first; next approve the exact config file; finally reread the tracker and preview
the issue create for its own approval. A read in that same repository skips setup
because it creates no durable shared choice.

### Cross-tracker request

The repository config selects Linear as canonical. A request begins from a
related GitHub issue. Resolve the Linear issue only through one exact native link
exposed by the provider, then preview and write the Linear issue alone. Linear's
[GitHub integration documentation](https://linear.app/docs/github) describes
the native link.
...
proposal (`skills/managing-issues/references/linear.md`, “Select one session
transport” and “Creates, surgical updates, and lifecycle”).
