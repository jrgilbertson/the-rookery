...
At the time, Repo Gardener's live authority was one durable yaml file in the
...
Gardener's fix at the time was PyYAML `SafeLoader` plus an allowlist:
boolean `true`/`false` only, decimal integers, fail-closed tags, aliases,
merge keys, nulls, and duplicate keys. The second reader went away: the
`lanes` subcommand was removed and the config fixture pinned the flow-style,
deeper-indent, and duplicate-key grammar cases against the validator
itself. Syntax restrictions used PyYAML composition events, rather than a
character scanner that must rediscover YAML quoting, comments, and scalar
rules. The field schema accepted lane mappings in any order and normalized
the result for consumers. SafeLoader is not the schema; `yes`, `0x10`, and
...
After: the validator's `parse_yaml_mapping` became the only parser, and the
fixtures covered four-space indent, flow mappings, block scalars, reordered
lanes, `yes` as a non-boolean, and duplicate keys. Both scripts have since
