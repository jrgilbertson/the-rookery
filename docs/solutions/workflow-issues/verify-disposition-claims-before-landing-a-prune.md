...
On branch `jrgilbertson/Agents-md-discoverability` (local history, unpushed at
time of writing), four suites' evidence-ledger test files (~6,000 lines) were
...
the author's memory is precisely what drifted:
...
- **False retirement — native-load provenance.** `875ba4a` recorded the native
  ...
  `06190b3`, which replaced the paragraph with the smoke-check-era
  same-name-collision caution.

All commit refs are local-branch history on
`jrgilbertson/Agents-md-discoverability`, unpushed at time of writing.
...
- [Skills CLI ref not checked out](../integration-issues/skills-cli-ref-not-checked-out.md)
  — shared meta-lesson: a verification that cannot distinguish success from
  silent fallback is not a verification.
