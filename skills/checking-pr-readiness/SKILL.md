...
description: Use when branch work looks complete and needs a readiness decision before another workflow opens a pull request, when asked whether the branch is ready to ship, or when asked to assess a specific head for PR readiness. Gathers the working surface and checks, then briefs a recommendation plus numbered live options and waits for a numbered reply. A reply of 1 is Approve. Do not pick an option in the same turn. A direct request to write, open, create, or submit a pull request belongs to PR publishing, not this skill. Do not use for existing-PR feedback, the pre-merge whole-change review, code review, simplification, plan review, general library production-readiness questions, or merging.
license: MIT
...
and wait for a numbered reply from whoever is talking. There is no separate
...
The finishing path will stage this surface. Run
...
statuses per the reference's helper exit map. Capture helper stdout into an
owner-only temp directory and do not echo the inventory into chat. Pass
`--full` so the listing written to temp is not capped. Resolve caps per class
11; one surface-report run.
...
size as covered by a repository gate). The spoken readout does not list every
path.
...
unavailable, stop rather than composing a readout from a surface you could
not read.

Internal completion: every path in all four categories is in the captured
surface report, or the run stopped because the working surface could not be
read from git. The spoken readout names untracked or blocking paths only when
they drive the recommendation.
...
Internal completion: the tracked and ignored enumerations cover every
resolved family; every transient hit is classified as ignored working
material, cleanup, or a finding; and every durable citation is accounted for.
If either enumeration is incomplete, stop rather than treating an incomplete
inventory as clean.
...
gates — a search for hook, script, and job names before any full-file read —
...
Internal completion: every discovered gate carries one status word and its
owner, and an empty discovery is a named finding in the gather. The spoken
brief names a gate only when it drives the recommendation.
...
exists, record not verified. In the spoken brief, name that step as next work
(for example, code review was not run). Do not ask anyone to vouch that it
happened. When the companion skill or tooling a check depends on is absent
(no compound engineering plugin, no design-critique tooling), record that
check skipped, name what was missing, and run the rest of the checks.

Internal completion: each of the five steps carries one status word in the
captured gather, every verified step names its receipt, and the user-interface
classification and its basis are stated. The spoken readout names only the
steps that drive the recommendation.
...
Internal completion: every planned item is marked delivered or not delivered
in the gather, or the comparison is recorded unavailable with intent
attestation. The spoken brief names only undelivered items or drift that
...
Internal completion: the gather carries exactly one of the three signals, and
any approval past an uncaptured learning carries the recorded reason.
...
that order. The spoken readout names only classes that drive the
recommendation.
...
Internal completion: every class in the reference carries one verdict from
that class's enumerated set in the captured gather, and each class that fired
...
- List only currently available options. Omit unavailable options.
- Clean green (approve and proceed, nothing material): final readout plus
...
- A coverage close: gather completed, and every applicable check is
  ...
  offer Approve.
- Scale which checks appear by applicability, never by how large the diff
  feels. Omit a check from the spoken readout when the working surface holds
  no path it covers. Paths touching authentication, authorization, payments,
  data migrations, secrets handling, or a published API contract stay visible
  when they have a finding or an incomplete check.

#### Decision menu

Present exactly one decision menu, then wait for a numbered reply. Do not
pick an option in the same turn that wrote the menu. A reply of `1`,
