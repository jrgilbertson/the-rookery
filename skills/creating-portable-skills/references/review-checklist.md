...
- The body fits its budget. Pass: `SKILL.md` is at most 500 lines.
- Context use is justified. Pass: required context fits declared host limits. Character count divided by four below 5,000 is an authoring target, not a failure threshold; justified required content may exceed the estimate.
...
- Completion criteria resist premature completion. Pass: each workflow stage closes on an exhaustive, checkable state where early completion would be visible, and no completion condition can be satisfied by declaring success without the required artifact or observable state.

## Instruction economy

- No-ops. Every line survives the delete test: ask of each line whether the model could already know it. Pass: what stays is what only the author knows (the audience, environment facts, the quality bar, resource contracts, hard judgment calls, and the reasons behind constraints), a System-Owned Invariant, or text whose removal would plausibly lose required behavior; text that only restates default reasoning is cut, and length alone never justifies a cut.
...
- Duplication. One meaning has one owner. Pass: a rule is defined in one place and cited elsewhere rather than paraphrased, and no sentence restates another sentence's meaning in the description, body, or bundled files.
...
- Frontmatter is canonical. Pass: only the fields in the **Package format** table of [skills.md](skills.md) appear, less any that the canonical-package rule in [portability.md](portability.md) leaves out.
