...
4. **A new review:** select one mode below. An explicit mode wins. Load
   ...
   map before the mode's source retrieval.
...
new review discovery. Load source bindings only when the approved target cannot
otherwise be resolved; stop that action if its binding remains unresolved.
