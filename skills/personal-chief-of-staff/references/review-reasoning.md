...
Use this reference for a new Wind-down, Weekly, or Quarterly review and for
its continuation, including Frontier Round answers and Wind-down Phase 2. It
owns what a review retrieves and how it reasons, coaches, and presents.
Retrieve only the evidence needed for the current review, then keep every
proposed change bound to the source that owns it.
[Source access](source-access.md) governs every read and write it makes.

## Contents

- [Retrieve the review baseline](#retrieve-the-review-baseline)
...
## Retrieve the review baseline

For each new Wind-down, Weekly, or Quarterly review, resolve the private map
through [source-bindings.md](source-bindings.md#resolve-before-recommending),
then read its configured strategy, durable learning, and bounded current task
commitments for the review's window before making recommendations. These three
baseline roles apply in every mode. Inspect returned current content for
applicability under
[Judge what returned evidence supports](source-access.md#judge-what-returned-evidence-supports).
Keep the configured task owner as the source of task state; neither the review
nor this map is a second task store.

The mode reference then names its incremental reads: its current record,
...
approved evidence requires a revised proposal and new approval.
...
Use the required [review-bundle asset](../assets/review-bundle.md) to present
the review. Lead with the answer, place evidence under the claim it supports,
and end with implications or independently approvable actions. Name
unavailable sources only where the gap changes confidence or scope. Before
numbering the first proposed action, read
[action-application.md](action-application.md).

A bundle is a conversational review surface, not a durable artifact.
