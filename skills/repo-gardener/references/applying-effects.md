...
The Orchestrator uses the `effect-v1` preparation and verification path
exactly twice: once for `run-opened`, then once for `run-closed`. It performs no
other managed tracker operation for that run ID. Workers never comment on the
tracker.

## Require one tracker writer

Before `run-opened`, the caller must ensure exclusive tracker-write ownership
or atomic serialization such that only one Orchestrator may mutate this
tracker. A model decision, lease check, or liveness read cannot establish that
guarantee. This skill defines no wrapper, lock, or exclusive-writer checker.
When the guarantee is absent or unknown, stop before any tracker write.
Read-only work may be reported to the caller, but the Orchestrator must not
mutate the tracker.
...
Orchestrator can read it after a process failure. If that state is unavailable,
stop before writing. This is recovery material, not another tracker record or a
...
`ambiguous` stop the dependent tracker sequence. A denied close is an
interrupted closure; do not invent a closed run.

## Recover an uncertain write

Never retry blindly. Reuse the original prepared object and operation ID.
Load them from the caller-owned recovery state when the original Orchestrator
is not available. Retain uncertain material for owner/recovery inspection;
discard it only after exact verification proves the operation complete or not
invoked.

- If body and comment already match, perform zero writes.
- If the body is the exact prepared body and the exact prepared comment is
  absent, append that comment once, then read every page again.
...
final snapshot. The command finds the durable opening comment in the final
snapshot, then checks only that this run ID has exactly two managed records,
opened then closed, with matching run identity, exact prepared closing
material, and final readback. Success is two-record identity, not a
planning-quality, safety, or register-consistency claim.
