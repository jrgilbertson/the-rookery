...
- the current issue body and stable issue identity;
...
`normalize-github-register` rejects incomplete pagination, unknown or duplicate
markers, unknown fields, identity mismatch, duplicate comment IDs, broken or
reordered history, comments ahead of the body, invalid rows, and projection or
size violations. A terminal line-feed difference around a managed receipt is
accepted, but the normalized canonical material is stable.
