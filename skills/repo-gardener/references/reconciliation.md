# Lead and Executor workflow

This reference reconciles repository, provider, and Executor facts during a
...
its matching `run-closed`. For the exact original Lead and its
Executors, require current caller or host liveness, or proven termination; an
...
Executors; the run still performs caller-only sensing and returns that result to
...
Executor mutation, and it preserves pending worktrees and authored state for
...
Each `run-opened` payload persists bounded, stable original Lead identity
...
enumerates the run's Executors. A run whose Executors cannot be enumerated from
...
never a Repo Gardener state machine or per-Executor tracker records.
...
or recommendation, including while selected Executors progress. Prefer credible
...
Immediately before every Executor dispatch, pass the revision check point.
Assignment names the files each Executor will touch, including any shared
convention file, so two Executors are not assigned the same one. This slice
...
that file owns everything the Executor does from setup through publication. The
Lead reads each checking-skill brief and, on a distinct later turn,
authorizes reply 1 only under the boundary sentences in `SKILL.md`.

## Supervision and review

After each supervised completion or Executor response, reread the current branch
and full head, diff, checks, PR, Reviewer brief, and relevant authority. Send
that same Executor every named Executor-owned gap those facts show. If none
...
Executor; never call it complete. Forward a named Executor-owned gap only when
local head, hosted PR head, and Executor authority still match the assessed
