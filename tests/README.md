...
- `triggers.md` holds the trigger contract: should-trigger and near-miss query
  tables with expected judgments and one-line reasons. Current contract only,
  no run history.
- `cases/<case-name>.md` holds one runnable behavioral case: the full prompt,
  any input files by relative path, a binary expected-behavior checklist, and
  one provenance line naming the observed failure or baseline gap that
  motivated the case.
- `log.md` holds one line per run or check: `date | git rev | check | result |
  note`. The `git rev` field names the commit the run's working tree was
  based on, which is the parent commit when the change under test is not yet
...
- A case enters a suite only when a baseline run showed the bare model failing
  the behavior, or an observed failure motivated it. Name that reason in the
  provenance line. Exception: a safety or privacy invariant may keep one case
  even when the bare model currently passes it.
- Roughly 10-15 active cases per skill is a ceiling for initial mining, not a
  target; suites grown by the observed-failure rule tend to stay smaller.
...
| — | per-harness smoke test | packaging or install-path change |
...
- **Trigger suite.** Judge each query in a fresh context that sees only the
  ...
  categorical `no` votes; a completed three-run set without two categorical
  same-side votes fails. Any near-miss `yes` fails the suite immediately.
...
- **Matched comparison (new skills and behavior-changing revisions).** A
  ...
  or substantive revision runs its affected cases as matched pairs, one
  without the skill (or with the frozen prior version) and one with the
  revised version, each in a fresh context. It ships only when the
  discriminating cases show the intended improvement with no regression. Log
  one line per graded variant.
- **Smoke test.** Install the skill from source into a disposable project on
  ...
  the smoke test once against the published default branch and log that line
  too. Installers pull from that branch, and local-source success does not
  prove remote resolution.

## Honest claims

A trigger-suite pass is a proxy measure, not proof of native triggering in a
harness; only a smoke test shows that. A log line states only what its run
actually checked.
