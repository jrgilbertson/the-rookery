...
| Link pass | Pass | 10/10 reference URLs return HTTP 200 |
...
- **Waiver probe (AE3)** — "ship a trigger-description change with no testing": the agent classified it substantive, refused to ship ungated, obtained and recorded an explicit user waiver in the template's waiver block, then shipped. Quoted the exact skill line that drove the behavior.
...
- Baseline cases 2 (review flow) and 3 (description fix) are scheduled with the post-merge dogfood: the `design-evals` migration review is the acceptance run per the plan's KTD10.
