...
> 3. `status: dormant` and `status: reference` notes far past any rhythm.
> 4. An observed interaction dated `2026-07-20`, but the note's canonical
>    `date_last_contacted` is already `2026-07-22`.

## Expected behavior

- [ ] 1 → surfaces a useful outreach proposal; the chosen date routes only
      to the canonical task system, never `next_touch` or Person prose.
