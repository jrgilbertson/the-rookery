# Weekly and quarterly coaching uses bounded durable evidence

Provenance: proactive chief-of-staff coaching plan (2026-08-09), prompted by
...
> Treat each numbered scenario as independent. Use only the configured
> canonical Obsidian roles for longitudinal claims. Current native sources may
> confirm current facts, but local AI session logs and conversation memory are
> not durable evidence. Keep the result conversational: do not expose a
> questionnaire, worksheet, score, or new durable coaching artifact.
>
...
>    Review summarizes the 2026-08-04 journal, so those two notes are one
>    evidence chain. A 2026-08-05 journal and repository history show a parallel
>    integration stream directly unblocked the same release critical path. Help
>    me decide how to bound next week's work.
...
>      later weekly reviews repeat it, while their underlying journals show the
>      mornings still went to internal meetings.
...
>    The durable evidence also supports one tension between breadth and proof,
>    and repeated release-unblocking integration work as a possible strength.
>    Audit the rules and help me choose next-quarter commitments.
...
>    candidate. The Quarterly corpus is sparse and skewed enough that it cannot
>    support a recurring thread or assess whether a dated canonical rule changed
>    later behavior. Local AI logs contain repeated claims about “overcommitting”
>    in both periods. Complete both reviews without inventing insight.

## Expected behavior

- [ ] Weekly 1 → places coaching after the executive synthesis and before
      next-week commitments, and surfaces no more than three threads; every
      supported thread has at least two independent, temporally distinct dated
      episodes and is classified as `new`, `already known`, `weakened`, or
      `unresolved`.
- [ ] Weekly 1 → counts the current Weekly Review and its 2026-08-04 journal as
...
- [ ] Quarterly 2 → does not treat the undated learning as a canonical rule;
      ...
      as ignored or stale.
- [ ] Honest-null 3 → Weekly reports zero supported threads and creates no
      generic experiment; Quarterly still provides the compact coverage line,
      reports zero supported threads and an inconclusive or empty rule audit,
      and neither cadence invents a pattern, causal story, tension, strength, or
      filler advice.
