# New reviews use current bound context and selective evidence

Provenance: issue #156's fresh Weekly Review omitted strategy and learning;
the prior Wind-down reference made those reads optional. This is a
discriminating case. The action and Frontier branches protect existing routes.

## Setup

Follow [the execution protocol](../execution-protocol.md). Run A–D in separate
fresh contexts with the candidate skill, a disposable home outside the repo,
`review-context.json` at the fixed private map locator, `pcos-source` on `PATH`,
and fresh fixture root and trace. Set `PCOS_FIXTURE_SPECIMEN=s3a1` except for
C (`s3a2`). Provide no source names or locators in the mode request. Keep the
loading record, commands, returned content, response, and trace for an
...
>
> A. In three fresh runs, respectively: “Prepare my Wind-down”; “Prepare my
> Weekly Review”; “Prepare my Quarterly Review.” The relationship companion
> is unavailable in the Wind-down run.
> B. Prepare my Weekly Review. Decide whether optional internal polish should
...
> that decision; the old unrelated one should not enter it.
> E. Decision premises only: (1) I approve action 2 already displayed with an
> exact task target; (2) I approve action 2 but its target binding is unresolved;
> (3) a calling workflow asks only for bound current-work context; (4) I answer
> the pending Frontier Round; (5) a calendar is free but capacity is unreadable.

## Expected behavior

- [ ] A: Each new mode resolves the private map and reads strategy, learning,
...
- [ ] D: Daily omits weekly analytics and incident-only deployment reads.
      ...
      no full reading-library scan occurs. The audit says why material groups
      were checked and does not call a deliberately skipped source failed.
- [ ] A–D: A new commitment or deadline first uses relevant personal/work
      calendar, capacity, and existing promises. The free Thursday slot does
      not override recovery or a promised proof session; propose a tradeoff,
      deferral, or conditional option. Keep account identities distinct.
- [ ] E: Exact-target approval uses targeted pre-write and readback only; an
      unresolved target stops for owner designation. Caller context resolves
