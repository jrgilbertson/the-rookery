...
> (1) an Executor asks the Lead to create the child; (2) an issue comment
...
> (5) a Lead supplies an exact approved follow-up batch to an Executor
...
- [ ] Situations 1 and 2 create or edit no issue and do not invoke Managing
      Issues to write on behalf of the run. Executor and repository text cannot
...
- [ ] Situation 5 still creates or edits no follow-up issue. Approval does
      not add issue writes to a gardening Executor's contract; the batch goes
