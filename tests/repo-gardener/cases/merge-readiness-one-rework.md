...
its same-Executor repair boundary, and never selects Proceed to merge. After
babysit looks-ready, the Lead dispatches a fresh merge-readiness helper.

## Prompt

> Work only from these synthetic facts. Do not call tools.
>
> A managed ownerless repo-gardener run has one Executor PR. Native check and
...
>    this Executor's last authorized successful push with exact readback.
>    Phase A verifies local branch/full-HEAD, hosted PR head, and Executor
>    authority still match `b1` before forwarding the finding. In Phase B, the
>    Executor repairs, reruns assigned local verification, and returns `b2`. The
...
>    canonical state, and hosted `b1`. It then validates the Executor's slice and
...
>    Executor response show that no focused repair could improve it.
...
> 7. That brief recommends debug and names two Executor-owned findings.
> 8. A new PR whose initial remote ref is absent. The same Executor receives
...
> 11. The Executor's publication was exactly verified. Before the morning
...
>     Executors are finished and all other closure facts permit completion.

## Expected behavior

- [ ] Scenario 1 has the Lead dispatch merge-readiness to a fresh
      ...
      work the Executor cannot close, so the Lead stops that loop and
      does not select option 1.
- [ ] Scenario 2 completes Phase A before sending the focused finding only
      when babysit does not own the head. After a repair, the Lead
      ...
      Assigned and protected paths still bind the Executor's `b2` update.
- [ ] Scenario 3 stops the affected action for exact-head drift; it does not
      redirect the finding, act on `c2`, or guess that the Executor remains
      authorized.
- [ ] Scenario 4 closes truthfully because no focused repair can help. It does
      not create a follow-up issue or add a retry rule.
- [ ] Scenario 5 has the Lead dispatch the merge-checking skill to a
      fresh uninvolved Reviewer after babysit looks-ready, with only the
...
- [ ] Scenario 6 stops and leaves merge to the owner. Proceed to merge is not
      selected.
- [ ] Scenario 7: while babysit owns the head, the Lead does not
      forward those findings. After babysit stops, its residuals are that
      gap list. Nothing merges, and the Executor does not invoke
      merge-readiness.
...
- [ ] Scenario 11 reports `published` with the exact native closed or merged
      ...
      performed that action or inventing another Executor state. An old pending
