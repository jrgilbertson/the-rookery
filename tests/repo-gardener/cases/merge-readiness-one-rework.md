...
| Post-return base retarget | The report-only result carries `release/2026.08` at `B1`; its head remains `H` and findings are otherwise actionable in slice. | After that result returns but before the consumer grants a batch, a fresh native PR read preserves `H` but reports `release/2026.09` at `B2`; it no longer matches either the authorized subject or returned assessment tuple. | Stop as `UNKNOWN` and grant no repair authority. A matching head alone cannot authorize findings against the retargeted cumulative diff. |
...
- [ ] That agent-mode subject carries the same authorized head repository and
      exact Worker branch plus the authorized base ref and exact base commit OID
      as the delivery tuple, and returns that base tuple for the post-return
      consumer comparison. Synthetic OPEN PR evidence with the same head OID
      and base tuple but a forked head repository or different Worker branch,
      as well as a base retarget without a head move, is `UNKNOWN` at initial
      validation or final comparison; neither repo-gardener caller may omit a
      supplied head-ref or base tuple.
