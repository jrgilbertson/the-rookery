...
> lane enabled. The assigned change touches only `src/safe.py`; all other
...
> policy identity, capacity zero, missing capacity, disabled owning lane,
...
- [ ] Brief distinguishes target identity from the opening policy's identity
      and carries its revision, capacity, scope, protected paths, and lane grant.
...
- [ ] Disabled lane, out-of-scope path, and protected path also deny authoring;
