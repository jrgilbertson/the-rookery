# Unattended PR-readiness menu and Lead loop

Provenance: an unattended Executor gathers through the normal PR-readiness
conversation; the Lead authorizes publish or sends named Executor-owned
gaps.
...
>
> A valid `.agents/repo-gardener.yaml` allows one Executor. It finished
...
>
> 1. The unattended Executor invokes `checking-pr-readiness` on that exact head.
...
>    all pass. The Executor does not choose option 1 on its own.
> 3. The brief withholds Approve and names two Executor-owned gaps. In a
...
> 5. The brief needs owner attestation and names no Executor-owned gaps the
>    Executor can close. Gather is otherwise complete.
> 6. The Executor receives an actionable native PR review finding, makes one
>    focused repair on its own PR, reruns its assigned verification, and returns
>    `b2`. The Lead sends that Executor-owned gap. The installed publisher
...
- [ ] Scenario 1 writes the normal brief and numbered menu, then stops. It
      neither pushes nor opens a PR in the menu turn. The Executor does not
      choose option 1 on its own.
- [ ] Scenario 2 continues only if the Lead authorizes reply `1`.
      The Executor does not choose option 1. The checking skill then rereads
      ...
      Executor then invokes `ce-babysit-pr mode:pipeline` and does not
      ...
      commit.
- [ ] Scenario 3 does not open a PR. The Lead sends both named
      Executor-owned gaps to that Executor. After a new exact head, PR-readiness
      runs again.
...
- [ ] Scenario 6 sends the named Executor-owned review finding back to that
      Executor. After the repair, the Lead authorizes that exact
      ...
      update, preserves the local commit, and does not retry. The Executor
...
- [ ] Scenario 8: babysit reports looks merge-ready. The Lead dispatches
      `checking-merge-readiness` to a fresh uninvolved Reviewer with only the
